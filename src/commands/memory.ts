...
export function registerMemoryCommands(program: Command): void {
  ...
  memory
    ...
    .action(async (opts) => {
      try {
        ...
        if (opts.codeFlow) {
          payload.metadata = { code_flow: opts.codeFlow.split(',').map((s: string) => s.trim()) };
        }
  ...
  memory
    ...
    .action(async (id: string, opts) => {
      try {
        ...
        const { data } = await api.get<{ data: Memory }>(`/memories/${id}`, params);
        spinner.stop();
  ...
  memory
    ...
    .option('--type <type>', 'Type (note/code_snippet/link/idea/documentation)')
    .option('--code-flow <chain>', 'Comma-separated call chain (e.g. "A.method(),B.method(),C.method()")')
    .action(async (id: string, opts: { title?: string; content?: string; type?: string; codeFlow?: string }) => {
      try {
        const api = createApiClient();
        ...
        if (opts.title || opts.content || opts.type || opts.codeFlow) {
          ...
          // Interactive mode
          const spinner = ora('Fetching memory...').start();
          const { data: current } = await api.get<{ data: Memory }>(`/memories/${id}`);
          spinner.stop();
  ...
  memory
    ...
    .action(async (query: string) => {
      try {
        ...
        const { data } = await api.get<{ data: Memory[] }>('/memories/search', {
          q: query,
  ...
  memory
    ...
    .action(async () => {
      try {
        ...
        const { data } = await api.get<ApiResponse<StatsData>>('/memories/stats', {
          project_id: projectId,
        });
        spinner.stop();
