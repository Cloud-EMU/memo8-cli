...
export function registerTestPatternCommands(program: Command): void {
  ...
  testPattern
    ...
    .action(async (opts) => {
      try {
        ...
        const { data } = await api.get<PaginatedResponse<TestPattern>>(
          `/projects/${projectId}/test-patterns`,
          params
        );
        spinner.stop();
        ...
        const rows = data.data.map((tp: TestPattern) => [
          String(tp.id),
  ...
  testPattern
    ...
    .action(async (opts: { patternName?: string; testType?: string; description?: string; templateCode?: string; applicableTo?: string }) => {
      try {
        const projectId = getProjectId();
        if (!projectId) {
          warn('No project specified. Run: memo8 init');
        ...
        let applicableTo: string[] | undefined;
        ...
        if (opts.patternName && opts.templateCode) {
          ...
          if (opts.applicableTo?.trim()) {
            applicableTo = opts.applicableTo.split(',').map((t: string) => t.trim()).filter(Boolean);
          }
        } else {
          // Interactive mode
          const answers = await inquirer.prompt([
            ...
            },
            {
              ...
              validate: (v: string) => (v.trim().length > 0 ? true : 'Template code is required'),
            },
            {
              type: 'input',
              name: 'applicable_to',
          ...
          if (answers.applicable_to?.trim()) {
            applicableTo = answers.applicable_to.split(',').map((t: string) => t.trim()).filter(Boolean);
          }
        }
        ...
        if (applicableTo && applicableTo.length > 0) payload.applicable_to = applicableTo;

        const { data } = await api.post<ApiResponse<TestPattern>>(
          `/projects/${projectId}/test-patterns`,
          payload
        );
        spinner.stop();
  ...
  testPattern
    ...
    .action(async (id: string) => {
      try {
        ...
        const { data } = await api.get<{ data: TestPattern }>(
          `/projects/${projectId}/test-patterns/${id}`
        );
        spinner.stop();
  ...
  testPattern
    ...
    .action(async (taskId: string, opts: { stdout?: boolean }) => {
      try {
        const projectId = getProjectId();
        if (!projectId) {
          warn('No project specified. Run: memo8 init');
        ...
        const { data } = await api.post<ApiResponse<{ testCode: string; patternName: string; testType: string }>>(
          `/projects/${projectId}/test-patterns/generate-for-task/${taskId}`
        );
        spinner.stop();
