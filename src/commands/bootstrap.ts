...
function walkDirectoryForTree(dir: string, ig: Ignore, baseDir: string, depth: number = 0, maxDepth: number = 4): string[] {
...
function getGitLog(dir: string, count: number): string {
  try {
    return execSync(
...
function getMostChangedFiles(dir: string, limit: number): string[] {
  try {
    const raw = execSync(
      `git log --name-only --format="" --diff-filter=ACMR | sort | uniq -c | sort -rn | head -${limit}`,
      { cwd: dir, encoding: 'utf-8', timeout: 15000, stdio: ['pipe', 'pipe', 'pipe'] }
    ).trim();
...
function findEntryPoints(dir: string, ig: Ignore): string[] {
  const found: string[] = [];

  function search(searchPath: string, depth: number): void {
    if (depth > 3) return;
    let entries;
    try {
...
function readFileTruncated(filePath: string, maxLines: number): string {
  try {
    ...
    if (stat.size > MAX_FILE_SIZE) return '[file too large]';
...
async function runStackScan(projectId: number, dir: string): Promise<boolean> {
  ...
  await api.post<ApiResponse<TechStack>>(
    ...
    .action(async (opts) => {
      try {
        ...
        for (const ep of entryPoints) {
          if (filesToRead.size >= maxFiles) break;
          filesToRead.add(ep);
        }
        ...
        sections.push(
          `## File Structure (${fileCount} files, ${dirCount} directories)\n\n` +
          '```\n' + fileTree.join('\n') + '\n```'
        );
        ...
        if (gitLog) {
          ...
          sections.push(
            `## Git History (last ${logLines.length} commits)\n\n` +
            '```\n' + gitLog + '\n```'
          );
        }
        ...
        if (fileContents.length > 0) {
          ...
          sections.push(
            `## Key File Contents (${fileContents.length} files)\n\n` +
            fileBlocks.join('\n\n')
          );
        }
