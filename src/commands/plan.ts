...
function printPlanSummary(plan: AiPlan): void {
  console.log();
...
function printPlanPreview(plan: AiPlan): void {
  ...
  planData.sections.forEach((section: PlanSection, si: number) => {
    ...
    section.tasks.forEach((task: PlanTask, ti: number) => {
...
export function registerPlanCommands(program: Command): void {
  ...
  plan
    ...
    .action(async (opts: { project?: string; prompt?: string; autoApprove?: boolean }) => {
      try {
        ...
        const { data: usageResp } = await api.get<ApiResponse<PlanUsage>>('/ai/plans/usage');
          ...
          const answers = await inquirer.prompt([
            {
              ...
              validate: (v: string) => (v.trim().length >= 10 ? true : 'Please provide at least 10 characters'),
            },
          ]);
        ...
        const { data } = await api.post<ApiResponse<AiPlan>>('/ai/plans', {
            ...
            const { modification } = await inquirer.prompt([
              {
                ...
                validate: (v: string) => (v.trim().length > 0 ? true : 'Please describe your changes'),
              },
            ]);

            const modifySpinner = ora('Modifying plan...').start();
            const { data: modifiedData } = await api.post<ApiResponse<AiPlan>>(
              `/ai/plans/${currentPlan.id}/modify`,
              { prompt: modification.trim() }
            );
            modifySpinner.stop();
  ...
  plan
    ...
    .action(async (opts) => {
      try {
        ...
        const { data } = await api.get<PaginatedResponse<AiPlan>>('/ai/plans', params);
        spinner.stop();
  ...
  plan
    ...
    .action(async (id: string) => {
      try {
        ...
        const { data } = await api.get<{ data: AiPlan }>(`/ai/plans/${id}`);
        spinner.stop();
  ...
  plan
    ...
    .action(async () => {
      try {
        ...
        const { data: resp } = await api.get<ApiResponse<PlanUsage>>('/ai/plans/usage');
        spinner.stop();
