...
import { success, warn, info, handleError, printTaskList, printTaskDetail, printPagination, printStats, } from '../lib/output.js';
export function registerTaskCommands(program) {
    ...
    task
        ...
        try {
            ...
            const { data } = await api.get(`/tasks/${id}`, params);
            spinner.stop();
    ...
    task
        ...
        try {
            ...
            if (opts.title || opts.description !== undefined || opts.status || opts.priority) {
                ...
                // Interactive mode
                const spinner = ora('Fetching task...').start();
                const { data: current } = await api.get(`/tasks/${id}`);
                spinner.stop();
                const answers = await inquirer.prompt([
                    {
                        type: 'input',
    ...
    task
        ...
        try {
            ...
            const { data: parent } = await api.get(`/tasks/${parentId}`);
            spinner.stop();
            let title;
            let description;
            let priority;
    ...
    task
        ...
        try {
            ...
            const api = createApiClient();
            const { data } = await api.get(`/projects/${projectId}/tasks/dependency-graph`);
            spinner.stop();
    ...
    task
        ...
        try {
            ...
            const api = createApiClient();
            const { data } = await api.get(`/projects/${projectId}/tasks/next`);
            spinner.stop();
