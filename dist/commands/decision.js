...
export function registerDecisionCommands(program) {
            ...
            if (opts.title && opts.decision) {
                ...
                const answers = await inquirer.prompt([
                    ...
                    },
                    {
                        ...
                        validate: (v) => (v.trim().length > 0 ? true : 'Decision is required'),
                    },
                    {
                        type: 'confirm',
                        ...
                        const alt = await inquirer.prompt([
                            {
                                ...
                                validate: (v) => (v.length > 0 ? true : 'Option is required'),
                            },
                            {
                                type: 'input',
                                name: 'reason',
                ...
                if (moreAnswers.consequences?.trim())
                    payload.consequences = moreAnswers.consequences.trim();
                if (moreAnswers.tags?.trim()) {
                    payload.tags = moreAnswers.tags.split(',').map((t) => t.trim()).filter(Boolean);
                }
            }
            const spinner = ora('Creating decision...').start();
            const api = createApiClient();
            const { data } = await api.post(`/projects/${projectId}/decisions`, payload);
            spinner.stop();
    ...
    decision
        ...
        try {
            ...
            const api = createApiClient();
            const { data } = await api.get(`/projects/${projectId}/decisions/${id}`);
            spinner.stop();
            const d = data.data;
            console.log();
    ...
    decision
        ...
        try {
            ...
            const api = createApiClient();
            const { data } = await api.get(`/projects/${projectId}/decisions/search`, { q: query });
            spinner.stop();
            if (data.data.length === 0) {
