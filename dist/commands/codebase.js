...
function walkDirectory(dir, ig, baseDir) {
    const files = [];
...
export function registerCodebaseCommands(program) {
    ...
    codebase
        ...
        try {
            ...
            const api = createApiClient();
            const { data } = await api.get(`/projects/${projectId}/codebase/search`, { q: query });
            spinner.stop();
    ...
    codebase
        ...
        try {
            ...
            const api = createApiClient();
            const { data } = await api.get(`/projects/${projectId}/codebase/status`);
            spinner.stop();
    ...
    codebase
        ...
        try {
            ...
            if (name)
                params.name = name;
            const { data } = await api.get(`/projects/${projectId}/codebase/symbols`, params);
            spinner.stop();
            if (data.data.length === 0) {
